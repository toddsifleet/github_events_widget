...
parse_data = function(data) {
  var event_data, push_events, _i, _len, _results;
  push_events = data.filter(function(x) {
    return x.type === "PushEvent";
  });
  _results = [];
  for (_i = 0, _len = push_events.length; _i < _len; _i++) {
    event_data = push_events[_i];
    _results.push(new GitPush(event_data));
  }
  return _results;
};
